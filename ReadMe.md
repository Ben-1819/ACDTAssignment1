...
### Parameters
There is only one parameter required and that is the symbols header, the symbol that will be put in the parameter will be selected from a dropdown, butbecause the CryptoSymbols API is premium only I will not be adding all available crypto symbols.
    
### Headers
There is one header that is required with this API and that is the X-Api-Key header, it will be filled in automatically using the CRYPTO_KEY variable fromthe environment file
### Response
...
16. clouds.all
    - Cloud coverage in a percentage